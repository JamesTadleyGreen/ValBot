...
def last_to_die(match):
    ...
    for player in match['players']['red']:
        d['Red'].append({'name':player['name'] + '#' + player['tag'],'pussy_score': 0 ,'rotate_score': 0})
    for player in match['players']['blue']:
        d['Blue'].append({'name':player['name'] + '#' + player['tag'],'pussy_score': 0 ,'rotate_score': 0})
    for i, round in enumerate(match['rounds']):
            ...
            for kill in player_stat['kill_events']:
                for player in d[losing_team]:
                    if kill['victim_display_name'] in player['name']:
                        deaths.append({'time':kill['kill_time_in_round'], 'name':kill['victim_display_name']})                        
        deaths.sort(key=lambda d : d['time'])
        if len(deaths) >= len(d[losing_team]):
            for player in d[losing_team]:
                if player['name'] == deaths[-1]['name']:
                    if (i <= 12 and losing_team == "Red") or (i > 12 and losing_team == "Blue"): #initial attack = Red
        ...
        elif len(deaths) == len(d[losing_team])-1: # If someone hasn't died on the losing team
            for player in d[losing_team]:
                died = False
                for death in deaths:
                ...
                if not died: # For the person who didn't die
                    if (i <= 12 and losing_team == "Red") or (i > 12 and losing_team == "Blue"): # If they're an attacker
                        #TODO might be not a bitch move, i.e. saving.
                        player['pussy_score'] += 1
                    else: # If they're a defender
                        if round["plant_events"]["plant_time_in_round"] < deaths[-1]['time']: #Has to have been planted for a loss
                            player['rotate_score'] += 1
                    break
